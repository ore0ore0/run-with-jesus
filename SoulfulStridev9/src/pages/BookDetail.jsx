...
export default function BookDetail({ lang='en' }) {
  ...
  const handleUpload = async (e) => {
    ...
    if (!file) return
    const res = await uploadReviewFile({ file, bookId: id })
    setMsg(`Uploaded: ${res.url}`)
    setShowUpload(false)
  }
    ...
    <div className="pt-16 mx-auto max-w-6xl px-4">
      <div className="grid grid-cols-1 md:grid-cols-[2fr_minmax(0,3fr)] gap-0">
        ...
        <section className="separator p-6 font-read">
          ...
          <div className="flex gap-3 mb-6">
            <button onClick={()=>setShowEditor(v=>!v)} className="px-4 py-2 rounded-md font-semibold bg-black/5">{t.writeReview}</button>
            <button onClick={()=>setShowUpload(true)} className="px-4 py-2 rounded-md font-semibold bg-black/5">{t.uploadReview}</button>
          </div>

          {showEditor && <ReviewEditor lang={lang} onSave={(txt)=>{ setMsg('Saved review (local)'); setShowEditor(false)}} />}

          {msg && <div className="mt-4 p-3 bg-black/5 rounded text-sm">{msg}</div>}
        </section>
      </div>
      ...
      <Modal open={showUpload} onClose={()=>setShowUpload(false)} labelledBy="upload-review-title">
        <div className="form-shell">
          ...
          <input type="file" accept=".pdf,.doc,.docx" onChange={handleUpload} className="border rounded-md p-3 w-full bg-white"/>
        </div>
      </Modal>
    </div>
  )
