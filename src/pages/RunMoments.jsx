...
export default function RunMoments({ lang='en' }) {
  ...
  useEffect(()=>{
    try { const raw = localStorage.getItem('runItems'); if (raw) setItems(JSON.parse(raw)) } catch {}
  },[])
  useEffect(()=>{ try { localStorage.setItem('runItems', JSON.stringify(items)) } catch {} }, [items])

  const onUploaded = (res) => setItems(prev => [res, ...prev])

  return (
    <div className="pt-16 mx-auto max-w-6xl px-4">
      <div className="grid grid-cols-1 md:grid-cols-[2fr_minmax(0,3fr)] gap-0">
        <section className="run-bg p-6 font-run" style={{minHeight:'calc(100vh - 4rem)'}}>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight mb-4">{t.gallery}</h1>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {items.map(it => (
              <figure key={it.id} className="rounded-lg overflow-hidden shadow bg-white/10">
            ...
            <button onClick={()=>setOpen(true)} className="h-40 rounded-lg border-2 border-dashed border-borderNeutral/50 flex items-center justify-center text-3xl" aria-label="Add" title="Add">+</button>
          </div>
        </section>
